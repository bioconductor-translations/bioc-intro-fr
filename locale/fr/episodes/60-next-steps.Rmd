...
- **One (or more) assay(s)** containing the quantitative omics data
  (expression data), stored as a matrix-like object. dans lesquelles les lignes représentent les _features_ (les variables mesurées, comme par exemple les gènes dans le cas d'une analyse RNAseq) et les colonnes représentent les différents échantillons analysés.
...
lack of variable type definitions). De plus, l'exportation de données vers une feuille de calcul
...
information, but are simply structured differently. Chaque structure a ses avantages propres: la première convient bien aux _packages_ `tidyverse`,
...
abstraction. Nous pouvons également voir dans la deuxième ligne affichée le nombre
